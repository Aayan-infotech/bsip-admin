    ...
    <div class="container-fluid py-3">
        <div class="row gx-5">
            ...
            <div class="col-md-12 content">
                <div>
                    <h3 id="notices" tabindex="0">{{ $language === 'hi' ? 'फॉर्म डाउनलोड करें' : 'Download Forms' }}</h3>
                ...
                <table class="table table-hover table-bordered align-middle" aria-describedby="notices">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th scope="col" class="text-center">{{ $language === 'hi' ? 'शीर्षक' : 'Title' }}<br><small class="text-muted">{{ $language === 'hi' ? '(शीर्षक)' : '(Title)' }}</small></th>
                            <th scope="col" class="text-center">{{ $language === 'hi' ? 'पीडीएफ' : 'PDF' }}<br><small class="text-muted">{{ $language === 'hi' ? '(पीडीएफ)' : '(PDF)' }}</small></th>
                            <th scope="col" class="text-center">
                                {{ $language === 'hi' ? 'डाउनलोड करें' : 'Word Document' }}
                                <br>
                                <small class="text-muted" style="color: #979797 !important;">{{ $language === 'hi' ? '(अंग्रेज़ी)' : '(English)' }}</small>
                            </th>
                            <th scope="col" class="text-center">
                                {{ $language === 'hi' ? 'डाउनलोड करें' : 'Word Document' }}
                                <br>
                                <small class="text-muted" style="color: #979797 !important;">{{ $language === 'hi' ? '(हिंदी)' : '(Hindi)' }}</small>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if ($notice->document)
                                <a href="{{ asset('storage/' . $notice->document) }}" target="_blank" class="btn btn-outline-success btn-sm" role="link" aria-label="{{ $language === 'hi' ? 'डाउनलोड करें' : 'Download' }}">
                                    <i class="fas fa-download" aria-hidden="true"></i> {{ $language === 'hi' ? 'डाउनलोड करें' : 'Download' }}
                                </a>
                                @else
                            ...
                            <td class="text-center">
                                ...
                                <a href="{{ asset('storage/' . $notice->hin_document) }}" target="_blank" class="btn btn-outline-success btn-sm" role="link" aria-label="{{ $language === 'hi' ? 'डाउनलोड करें' : 'Download' }}">
                                    <i class="fas fa-download" aria-hidden="true"></i> {{ $language === 'hi' ? 'डाउनलोड करें' : 'Download' }}
                                </a>
                                @else
                                <span class="text-muted">{{ $language === 'hi' ? '-' : '-' }}</span>
