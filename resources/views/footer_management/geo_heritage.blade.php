                    ...
                    <div class="card-header">
                        <nav aria-label="breadcrumb" class="float-start">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('bsip.geo.heritage') }}">BSIP Geo Heritage</a></li>
                            </ol>
                        </nav>
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Geo Heritage</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#addNews").on("submit", function(e) {
                ...
                grecaptcha.ready(function() {
                    grecaptcha.execute("{{ env('RECAPTCHA_SITE_KEY') }}", {
                        action: 'bsip_geo_heritage/store'
                    }).then(function(token) {
                        let formData = new FormData(form[0]);
                        formData.append("g-recaptcha-response", token);
                        $.ajax({
                            url: "{{ route('bsip.geo.heritage.store') }}",
                            type: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function(response) {
                                toastr.success(
                                    "Geo Heritage Event added successfully!"
                                );
                                form.trigger("reset");
                                table.ajax.reload();
                            },
            ...
            var table = $('#newsTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('bsip.geo.heritage.list') }}",
                    type: "GET",
                    data: function(d) {
                        d.customParam = 'value';
                    },
            ...
            $(document).on('change', '.toggle-status', function() {
                ...
                $.ajax({
                    url: "{{ route('bsip.geo.heritage.toggleStatus') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
            ...
            $(document).on('change', '.toggle-archived-status', function() {
                ...
                $.ajax({
                    url: "{{ route('bsip.geo.heritage.toggleArchivedStatus') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
            ...
            $(document).on('click', '.edit-bsip-geo-heritage', function() {
                let id = $(this).data('id');

                $.ajax({
                    url: "/bsip_geo_heritage/edit/" + id,
                    type: "GET",
                    success: function(response) {
            ...
            $('#editNewsForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "/bsip_geo_heritage/update/" + id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
